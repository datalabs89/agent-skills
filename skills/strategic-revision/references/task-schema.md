...
This document specifies the JSON format used by `dag_validator.py`. The file is first generated in Phase 3b (with `"block": "?"` placeholders) for structural validation, then updated with block assignments from Phase 4 before the Phase 6 full analysis.
